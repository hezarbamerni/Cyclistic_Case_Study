---
title: "Cyclistic Case Study Q3_2021"
author: "Hezar K"
date: "2022-11-29"
...
q3_2021$month <- month.name[as.numeric(q3_2021$month)]
...
```{r}
q3_2021 <- subset (q3_2021, ride_length > 59)
```
...
```{r}
q3_2021$day_of_week <- ordered(q3_2021$day_of_week, levels=c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"))
```
...
```{r}
rt<- as.data.frame(table(q3_2021$rideable_type,q3_2021$member_casual))
```
...
Create vector of month names for Q3 2021
septmeber 
```{r}
q3_months <- c("July", "August", "Septmber")
```

Subset month.name to include only Q3 2021 months

```{r}
q3_month_names <- month.name[match(q3_months, month.name)]
```

Create trips_by_month dataframe with only Q3 2021 months

```{r}
trips_by_month <- data.frame(month = q3_month_names, count = table(q3_2021$month))
```

Set the levels of the month variable in the trips_by_month dataframe

```{r}
trips_by_month$month <- factor(trips_by_month$month, levels = c("July", "August", "Septmber"))
ggplot(trips_by_month, aes(x = month, y = count.Freq)) +
geom_bar(stat = "identity", fill = "#F8766D") +
  labs(x = "Month", y = "Number of Rides", title = "Number of Rides by Month in Q3 2021")
```
