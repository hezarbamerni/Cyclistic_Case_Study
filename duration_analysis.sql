...
/*
+This query finds the maximum, minimum, and average ride duration for each combination of rider type (member_casual) and day of the week (day_of_week).
+It does this by first selecting the member_casual, day_of_week, and ride_length columns from the alltrips21v view, and then grouping the results by 
+member_casual and day_of_week. The maximum, minimum, and average ride durations are calculated using the MAX(), MIN(), and AVG() aggregate functions,
+respectively.
*/
...
/*
+This query finds the maximum, minimum, and average ride duration for each rider type (member_casual). It does this by selecting the member_casual and
+ride_length columns from the alltrips21v view, and then grouping the results by member_casual. The maximum, minimum, and average ride durations are again
...
/*
+This query finds the maximum, minimum, and average ride duration for each day of the week (day_of_week). +It does this by selecting the day_of_week and
+ride_length columns from the alltrips21v view, and then grouping the results by day_of_week. The maximum, minimum, and average ride durations are again
+calculated using the MAX(), MIN(), and AVG() aggregate functions.
*/
